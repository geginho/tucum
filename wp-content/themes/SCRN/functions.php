...
if( !function_exists('vp_load_css') ) {
    ...
    wp_enqueue_script( 'backstretch', get_template_directory_uri() . '/js/jquery.backstretch.js', array(), '1.0');
	}
}
...
function vp_custom_css() {
	global $teo_data;
	if(isset($teo_data['custom_css']) && $teo_data['custom_css'] != '')
			echo '<style type="text/css">' . $teo_data['custom_css'] . '</style>';
}

add_action( 'wp_ajax_teo-contact-form', 'teo_contact_form' );
add_action( 'wp_ajax_teo-contact-form', 'teo_contact_form' );
 
function teo_contact_form() {
