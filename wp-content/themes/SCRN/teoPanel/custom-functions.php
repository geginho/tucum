...
if( !function_exists( 'show_top_menu') )
{
	function show_top_menu() {
		...
		if(isset($teo_data['pages_topmenu']) && $teo_data['pages_topmenu'] != '' )
			$pages = get_pages( array('include' => $teo_data['pages_topmenu'], 'sort_column' => 'menu_order', 'sort_order' => 'ASC') );
		else
			$pages = get_pages('number=4&sort_column=menu_order&sort_order=ASC');
		$count = count($pages);
		if($teo_data['menu_homelink'] == '1') 
			echo '<li><a href="' . get_home_url() . '/#intro">Home</a>';
		for($i = 0; $i < $count; $i++)
		{
			echo '<li><a href="' . get_home_url() . '/#' . $pages[$i]->post_name . '">' . $pages[$i]->post_title . '</a></li>' . PHP_EOL;
		}
		if(isset($teo_data['blog_page']) && $teo_data['blog_page'] != '')
			echo '<li><a href="' . get_permalink($teo_data['blog_page'][0]) . '">Blog</a></li>';
...
if(! function_exists('vp_customization'))
{
	function vp_customization() {
		...
		if( (isset($teo_data['bg_image1']['url']) && $teo_data['bg_image1']['url'] != '') ||
			(isset($teo_data['enable_slideshow']) && $teo_data['enable_slideshow']) ) { 
			echo "#intro .bg1 { background-image: none; background-color: #000; opacity: 0.2; }  \n";
		}
		if(isset($teo_data['bg_color']) && $teo_data['bg_color'] != '') {
			echo "#intro .bg1 { background-image: none; background-color: " . $teo_data['bg_color'] . "; } \n";
		}
