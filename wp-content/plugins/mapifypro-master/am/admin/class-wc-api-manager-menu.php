...
class Mapify_AM_MENU {
			...
			?>
				...
				<?php
					if( $tab == MAPIFY_AM()->ame_activation_tab_key ) {
							...
							submit_button( __( 'Save Changes', MAPIFY_AM()->text_domain ) );
					} else {
	...
	public function load_settings() {
		...
		// API Key
		add_settings_section( MAPIFY_AM()->ame_api_key, __( 'API License Activation', MAPIFY_AM()->text_domain ), array( $this, 'wc_am_api_key_text' ), MAPIFY_AM()->ame_activation_tab_key );
		add_settings_field( MAPIFY_AM()->ame_api_key, __( 'API License Key', MAPIFY_AM()->text_domain ), array( $this, 'wc_am_api_key_field' ), MAPIFY_AM()->ame_activation_tab_key, MAPIFY_AM()->ame_api_key );
		add_settings_field( MAPIFY_AM()->ame_activation_email, __( 'API License email', MAPIFY_AM()->text_domain ), array( $this, 'wc_am_api_email_field' ), MAPIFY_AM()->ame_activation_tab_key, MAPIFY_AM()->ame_api_key );
		...
		add_settings_section( 'deactivate_button', __( 'API License Deactivation', MAPIFY_AM()->text_domain ), array( $this, 'wc_am_deactivate_text' ), MAPIFY_AM()->ame_deactivation_tab_key );
		add_settings_field( 'deactivate_button', __( 'Deactivate API License Key', MAPIFY_AM()->text_domain ), array( $this, 'wc_am_deactivate_textarea' ), MAPIFY_AM()->ame_deactivation_tab_key, 'deactivate_button' );
		...
		if ( $_REQUEST['option_page'] != MAPIFY_AM()->ame_deactivate_checkbox ) {
			...
			if ( $activation_status == 'Deactivated' || $activation_status == '' || $api_key == '' || $api_email == '' || $checkbox_status == 'on' || $current_api_key != $api_key  ) {
				...
				if ( $activate_results['activated'] == true ) {
					add_settings_error( 'activate_text', 'activate_msg', __( 'Plugin activated. ', MAPIFY_AM()->text_domain ) . "{$activate_results['message']}.", 'updated' );
					update_option( MAPIFY_AM()->ame_activated_key, 'Activated' );
					update_option( MAPIFY_AM()->ame_deactivate_checkbox, 'off' );
				}

				if ( $activate_results == false ) {
					add_settings_error( 'api_key_check_text', 'api_key_check_error', __( 'Connection failed to the License Key API server. Try again later.', MAPIFY_AM()->text_domain ), 'error' );
