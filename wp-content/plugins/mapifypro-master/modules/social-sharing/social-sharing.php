...
function mpfy_ss_mpfy_plugin_settings($custom_fields) {
	...
	$custom_fields['mpfy_load_sharethis'] = Carbon_Field::factory('select', 'mpfy_load_sharethis', 'Load ShareThis')
		->add_options(array(
			'y'=>'Yes',
			'n'=>'No',
		))
...
function mpfy_ss_map_location_custom_fields($custom_fields) {
	...
	$custom_fields['mpfy_load_sharethis'] = Carbon_Field::factory('set', 'map_location_share', 'Share Services')
		->add_options(array(
			'facebook'=>'Facebook',
			'pinterest'=>'Pinterest',
			'twitter'=>'Twitter',
...
function mpfy_ss_enqueue_assets() {
	...
	// load sharethis
	$load = carbon_get_theme_option('mpfy_load_sharethis');
	$load = $load ? $load : 'y';
	if ($load === 'y') {
		?>
