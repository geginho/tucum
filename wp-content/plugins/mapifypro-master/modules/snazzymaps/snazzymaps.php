...
function mpfy_sm_admin_behaviors($hook) {
    if ($hook !== 'post.php') {
        return;
    }

    wp_enqueue_script('mpfy-snazzymaps-admin', plugins_url('modules/snazzymaps/admin.js', MAPIFY_PLUGIN_FILE), array('jquery'), '1.0.0', true);
}
add_action('admin_enqueue_scripts', 'mpfy_sm_admin_behaviors');

// Add relevant admin fields
function mpfy_sm_map_custom_fields($custom_fields) {

	$custom_fields = mpfy_array_push_key($custom_fields, 'map_google_mode', array(
		'map_google_style'=>Carbon_Field::factory('select', 'map_google_style', 'Style')
...
add_filter('mpfy_map_custom_fields', 'mpfy_sm_map_custom_fields');

// Add style live-update to admin fields admin fields
function mpfy_sm_google_map_styles($styles) {

	$styles = json_decode(mpfy_sm_get_snazzymaps_js());

	return $styles;
}
add_filter('mpfy_google_map_styles', 'mpfy_sm_google_map_styles');

// Load stored value in front-end
function mpfy_sm_google_map_style_name($value, $map_id) {
	$value = get_post_meta($map_id, '_map_google_style', true);
	return $value;
}
add_filter('mpfy_google_map_style_name', 'mpfy_sm_google_map_style_name', 10, 2);

function mpfy_sm_google_map_style($value, $map_id) {
	$selected_style = get_post_meta($map_id, '_map_google_style', true);
	if ($selected_style) {
		$value = json_decode(mpfy_sm_get_snazzymaps_js($selected_style));
	}
	
	return $value;
}
add_filter('mpfy_google_map_style', 'mpfy_sm_google_map_style', 10, 2);

// Snazzymaps values
function mpfy_sm_get_snazzymaps() {
	return array(
		'default'=>'Default',
		...
		'lunar-landscape'=>'Lunar Landscape',
		'bright-&-bubbly'=>'Bright & Bubbly',
	);
}

function mpfy_sm_get_snazzymaps_js($style = null) {
	$snazzymaps = array(
	...
	if ($style === null) {
		return json_encode($snazzymaps);
	}

	if (isset($snazzymaps[$style])) {
		return $snazzymaps[$style];
	}
	return '[]';
}
