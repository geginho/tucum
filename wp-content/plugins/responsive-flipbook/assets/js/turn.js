...
turnMethods = {
	...
	updateOptions: function(optT, optP) {
		optP = typeof optP !== 'undefined' ? optP : {};
		var d = this.data(),
			pages = d.pages;

		for(var page in pages) {
			pages[page].data().pageFlip.opt = $.extend(pages[page].data().pageFlip.opt, optT, optP);
		}

		d.opt = $.extend(d.opt, optT);
	},

	updatePageOptions: function(optP) {
		var d = this.data(),
			pages = d.pages;

		for(var page in pages) {
			pages[page].data().pageFlip.opt = $.extend(pages[page].data().pageFlip.opt, optP);
		}
	},
	...
	init: function(opt) {
		...
		if ( opt.force_open )
			opt.page = Math.max( Math.min( opt.page, d.totalPages - 1 ), 2 );
	...
	page: function(page) {
		...
		if ( page && d.opt.force_open )
			update_corners( page, d.totalPages );
	...
	next: function() {
		...
		if ( ( ! this.data().opt.force_open ) || ( this.data().opt.force_open && ! prevent_flip( this.data().page, 'next', this.data().totalPages ) ) )
	...
	previous: function() {
		...
		if ( ( ! this.data().opt.force_open ) || ( this.data().opt.force_open && ! prevent_flip( this.data().page, 'previous', this.data().totalPages ) ) )
			turnMethods._moveTo.call(this, -1);
...
flipMethods = {
	...
	resize: function(full) {
		...
		if (d.parent.is(':visible')) {
			d.fwrapper.css({top: d.parent.offset().top, left: d.parent.offset().left});
			//d.fwrapper.css({top: Math.round(d.parent.offset().top), left: Math.round(d.parent.offset().left)});

			if (d.opt.turn)
				d.fparent.css({top: -d.opt.turn.offset().top, left: -d.opt.turn.offset().left});
				//d.fparent.css({top: Math.round(-d.opt.turn.offset().top), left: Math.round(-d.opt.turn.offset().left)});
		}
	...
	_displayCorner: function(p) {
		...
		var that = this,
			...
			transform = function(tr, c, x, a) {
				...
				// TODO: Fix the shadows for other browsers
				//var degA = Math.atan2((gradientEndPointA.x - (left?100:0) ) * -1, gradientEndPointA.y - (top?100:0)) * (180 / Math.PI) - 180;
				//var degB = Math.atan2((gradientEndPointB.x - (left?0:100) ) * -1, gradientEndPointB.y - (top?0:100)) * (180 / Math.PI) - 180;
				...
				if (d.opt.frontShadow)
					d.ashadow.css({'background-image':
									...
									//+',linear-gradient(' + degA + 'deg, rgba(0,0,0,0) ' + gradientStartV + ', rgba(0,0,0,'+(0.2*gradientOpacity)+') ' + (((1-gradientStartV)*0.8)+gradientStartV) + ', rgba(255,255,255,'+(0.2*gradientOpacity)+') 100% )'
					});
				...
				if (d.opt.backShadow)
					d.bshadow.css({'background-image':
									...
									//+',linear-gradient(' + degB + 'deg, rgba(0,0,0,0) 0%, rgba(0,0,0,'+(0.5*gradientOpacity)+') 80%'
					});
		...
		switch (p.corner) {
			case 'tl' :
				...
				d.fpage.transform(translate(-height, -width, ac) + rotate((90-a*2).toFixed(4)) , '100% 100%');
			...
			case 'tr' :
				...
				d.fpage.transform(translate(0, -width, ac) + rotate((-90+a*2).toFixed(4)) , '0% 100%');
			...
			case 'bl' :
				...
				d.fpage.transform(translate(-height, 0, ac) + rotate((-90+a*2).toFixed(4)), '100% 0%');
				d.opt.back.transform(rotate(270) + translate(-width, 0, ac), '0% 0%');
			...
			case 'br' :
				...
				d.fpage.transform(rotate((90-a*2).toFixed(4)), '0% 0%');
				d.opt.back.transform(rotate(90) + translate(0, -height, ac), '0% 0%');
	...
	turnPage: function() {
		...
		if ( d.opt.force_open )
			update_corners( d.opt.next, d.opt.turn.data().totalPages );
...
var prevent_flip = function( page, dir, total ) {
	var _flip = false;

	if ( page <= 3 && dir == 'previous' )
		_flip = true;

	if ( page + 2 >= total && dir == 'next' )
		_flip = true;

	return _flip;
};

var update_corners = function( page, total ) {
	if ( page <= 3 ) {
		corners = {
			backward: [],
			forward:  [ 'tr', 'br' ],
...
$( window ).on( 'rfbwp-update-corners', update_corners );

var disable_corners = function() {
	corners = {
		backward: [],
		forward:  [],
		all:      []
...
$( window ).on( 'rfbwp-disable-corners', disable_corners );
